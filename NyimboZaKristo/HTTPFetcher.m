...
- (void)close
{
	...
	/*we will instead handle response in handleResponse method
...
didReceiveResponse:(NSHTTPURLResponse *)aResponse
{
	...
	if ([aResponse statusCode] >= 400)
	{
		...
		{
			errorMessage =
            NSLocalizedStringFromTable(@"Sorry, the App does not have permission to access the service.", @"HTTPFetcher", @"Error given when a file permissions problem prevents you opening or playing a file.");
		}
		...
		{
			errorMessage =
            NSLocalizedStringFromTable(@"Oops! An internal server error occurred.", @"HTTPFetcher", @"Error given when an unknown problem occurs on the server.");
		}
		else
		{
			errorMessage =
            NSLocalizedStringFromTable(@"Sorry, there was an error in processing your request. Please try again later.", @"HTTPFetcher", @"Error given when an unknown problem occurs on the server.");
		}
...
- (void)connection:(NSURLConnection *)aConnection didFailWithError:(NSError *)error
{
	...
	if (showAlerts)
	{
		...
		if ([error code] == -1012)
		{
            errorMessage = @"Sorry, a client authentication error occurred.";
		}
		else if ([error code] == -1004)
		{
            errorMessage = @"Sorry, the application could not connect to the server.";
        }
		else if ([error code] == -1001)
		{
            errorMessage = @"Sorry, the server is taking too long to respond.";
		}
		else
		{
            errorMessage = @"Sorry, the application could not connect to the server.";
