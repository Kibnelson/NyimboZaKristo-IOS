...
- (UIToolbar *)getToolbarWithPrevNextDone:(BOOL)prevEnabled :(BOOL)nextEnabled height:(CGFloat)height
{
    ...
    if(height > 0.0f){
        toolbar.frame = CGRectMake(toolbar.frame.origin.x, toolbar.frame.origin.y, toolbar.frame.size.width, height);
    }
    NSMutableArray *itemsArray = [[[NSMutableArray alloc] init] autorelease];
    
    UISegmentedControl *tabNavigation = [[[UISegmentedControl alloc] initWithItems:[NSArray arrayWithObjects:@"Previous", @"Next", nil]] autorelease];
    ...
    UIBarButtonItem *barSegment = [[[UIBarButtonItem alloc] initWithCustomView:tabNavigation] autorelease];
    
    [itemsArray addObject:barSegment];
    
    UIBarButtonItem *flexButton = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil] autorelease];
    [itemsArray addObject:flexButton];
    
    UIBarButtonItem *doneButton =[[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(userClickedDone:)] autorelease];
